...
class HuggingFaceTokenizer(HuggingFaceSpecialToken, BaseTokenizer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # TODO: check whether we use Jinja template
        ...
        for file_id, file_path in vocab_files.items():
            ...
            if is_local:
                ...
                if os.path.exists(full_file_path):
                    ...
                    resolved_vocab_files[file_id] = None
                    print(f"Warning: {file_id} not found in {full_file_path}")
            else:
    ...
    def get_padding_and_truncation_strategy(
        ...
        **kwargs,
        ...
        if max_length is not None and padding is False and truncation is None:
            print("Warning: truncation is not explicitly set but "
            ...
            if padding is True:
                if max_length is not None:
                    if truncation is None or truncation is False or truncation == "no_truncate":
                        print("Warning: 'max_length' is ignored when `padding`=`True` and there is no truncation. "
        ...
        if max_length is None:
            if padding_strategy == PaddingStrategy.MAX_LENGTH:
                if self.model_max_length == INIT_MODEL_MAX_LEN:
                    print("Warning: `model_max_length` is not set, default to no padding.")
                    padding_strategy = PaddingStrategy.NO_PAD
                else:
            ...
            if truncation_strategy != TruncationStrategy.NO_TRUNCATE:
                if self.model_max_length == INIT_MODEL_MAX_LEN:
                    print("Warning: `model_max_length` is not set, default to no truncation.")
                    truncation_strategy = TruncationStrategy.NO_TRUNCATE
                else:
                    max_length = self.model_max_length
        ...
        **kwargs: Dict[str, Any]
        ...
        for encoding in encodings:
            ...
            if verbose and max_length is None and len(encoding.ids) > self.model_max_length:
                print(
                    f"Warning: Token indices sequence length is longer than the specified maximum sequence length for this model "
